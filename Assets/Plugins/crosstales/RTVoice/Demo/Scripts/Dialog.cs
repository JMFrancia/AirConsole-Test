         ...
         if (!Running)
         {
            ...
            while (Running && DialogPersonA != null && index < DialogPersonA.Length || DialogPersonB != null && index < DialogPersonB.Length)
            {
               ...
               if (Running)
               {
                  ...
                  uidSpeakerB = ModeB == Model.Enum.SpeakMode.Speak ? Speaker.Speak(CurrentDialogB, AudioPersonB, Speaker.VoiceForGender(GenderB, CultureB, 1), true, RateB, PitchB, VolumeB) : Speaker.SpeakNative(CurrentDialogB, Speaker.VoiceForGender(GenderB, CultureB, 1), RateB, PitchB, VolumeB);
