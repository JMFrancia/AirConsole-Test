...
namespace Crosstales.RTVoice.Provider
{
   ...
   public abstract class BaseCustomVoiceProvider : MonoBehaviour, IVoiceProvider
   {
      ...
      protected virtual void copyAudioFile(Model.Wrapper wrapper, string outputFile, bool isLocalFile = true,
         byte[] data = null)
      {
         if (wrapper != null)
         {
      ...
      protected virtual string getVoiceName(Model.Wrapper wrapper)
      {
         ...
         {
            if (Util.Config.DEBUG)
               Debug.LogWarning("'wrapper.Voice' or 'wrapper.Voice.Name' is null! Using the providers 'default' voice.");
